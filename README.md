...
<p>This component is the core binary of the daemon that is set up as a systemd socket on the monitored system. The service starts when a connection is detected and runs until no client acknowledgements have been received for a configurable amount of time. Clients initiate a connection to the daemon and receive a stream of system snapshots.</p>
